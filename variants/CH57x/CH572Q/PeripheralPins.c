...
/* =====
 * Notes:
 * - The CH572 peripheral pin mapping is based on the datasheet specifications
 * - CH572 uses a comparator which can act as a 4-bit ADC with 16 reference voltage levels
 * - CH572 has no true DAC, but supports PWM for analog output functions
 * - CH572 includes Bluetooth Low Energy (BLE) support not present in CH570
...
// CH572 doesn't have a true DAC, use PWM instead
...
#ifdef HAL_BLE_MODULE_ENABLED
// BLE specific pins (CH572 only)
const PinMap PinMap_BLE[] = {
    {PB_14, BLE, PIN_DATA(GPIO_AF_0, 0, 0)}, // RF Antenna connection
    {NC,    NP,  0}
};
#endif
